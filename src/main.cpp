...
void init_web_server() {
    ...
    server.on("/", HTTP_GET, [](AsyncWebServerRequest *request) {
        String html = "<!DOCTYPE html><html><head><title>ESP32 Fan Controller</title><style>"
            "body{font-family:Arial,sans-serif;background:linear-gradient(135deg,#1e3c72 0%,#2a5298 100%);color:white;padding:20px;margin:0}"
            ".container{max-width:800px;margin:0 auto;background:rgba(255,255,255,0.1);border-radius:15px;padding:25px;box-shadow:0 8px 32px rgba(0,0,0,0.3)}"
            "h1{text-align:center;margin:0 0 25px 0;font-size:28px;text-shadow:2px 2px 4px rgba(0,0,0,0.3)}"
            "#debug{background:rgba(255,100,100,0.3);padding:12px;margin:15px 0;border-radius:8px;font-size:13px;border:1px solid rgba(255,100,100,0.5)}"
            "#status{background:rgba(0,0,0,0.3);padding:15px;margin:15px 0;border-radius:10px;border:1px solid rgba(255,255,255,0.2)}"
            ".info{display:flex;justify-content:space-between;margin:8px 0;font-size:14px}"
            ".info span:first-child{font-weight:bold}"
            ".fan{background:rgba(255,255,255,0.15);margin:20px 0;padding:20px;border-radius:12px;border:1px solid rgba(255,255,255,0.2)}"
            ".fan h3{margin:0 0 15px 0;font-size:20px;color:#FFD700;text-shadow:1px 1px 2px rgba(0,0,0,0.5)}"
            ".buttons{display:grid;grid-template-columns:repeat(6,1fr);gap:8px;margin-top:15px}"
            ".btn{background:linear-gradient(145deg,rgba(255,255,255,0.2),rgba(255,255,255,0.1));border:1px solid rgba(255,255,255,0.3);color:white;padding:10px 6px;border-radius:8px;cursor:pointer;font-weight:bold;font-size:12px;transition:all 0.2s}"
            ".btn:hover{background:linear-gradient(145deg,rgba(255,255,255,0.3),rgba(255,255,255,0.2));transform:translateY(-1px)}"
            ".btn.active{background:linear-gradient(145deg,#00ff88,#00cc66);border:1px solid #00ff88;box-shadow:0 4px 15px rgba(0,255,136,0.3)}"
            ".rgb-section{background:rgba(255,215,0,0.1);padding:15px;margin-top:15px;border-radius:10px;border:1px solid rgba(255,215,0,0.3)}"
            ".rgb-section h4{margin:0 0 12px 0;color:#FFD700;font-size:16px}"
            ".rgb-buttons{display:flex;gap:10px;align-items:center}"
            ".rgb-btn{background:linear-gradient(145deg,rgba(255,255,255,0.2),rgba(255,255,255,0.1));border:1px solid rgba(255,255,255,0.3);color:white;padding:8px 16px;border-radius:6px;cursor:pointer;font-weight:bold;font-size:14px;min-width:40px;transition:all 0.2s}"
            ".rgb-btn:hover{transform:translateY(-1px)}"
            ".rgb-btn.red.on{background:linear-gradient(145deg,#ff4444,#cc3333);border-color:#ff4444;box-shadow:0 4px 15px rgba(255,68,68,0.4)}"
            ".rgb-btn.green.on{background:linear-gradient(145deg,#44ff44,#33cc33);border-color:#44ff44;box-shadow:0 4px 15px rgba(68,255,68,0.4)}"
            ".rgb-btn.blue.on{background:linear-gradient(145deg,#4444ff,#3333cc);border-color:#4444ff;box-shadow:0 4px 15px rgba(68,68,255,0.4)}"
            "select{padding:8px;border-radius:6px;background:rgba(255,255,255,0.2);color:white;border:1px solid rgba(255,255,255,0.3)}"
            ...
            "<div class=\\\"fan\\\">"
            "<h3>RGB Effects</h3>"
            "<select id=\\\"effect-select\\\" onchange=\\\"setArgbEffect(this.value)\\\">"
            "<option value=\\\"0\\\">Solid Colors</option>"
            ...
            "<h4>RGB LEDs (GPIO 16)</h4>"
            "<div class=\\\"rgb-buttons\\\">"
            "<button class=\\\"rgb-btn red\\\" id=\\\"fan1-red\\\" onclick=\\\"toggleFan1Red()\\\">R</button>"
            "<button class=\\\"rgb-btn green\\\" id=\\\"fan1-green\\\" onclick=\\\"toggleFan1Green()\\\">G</button>"
            "<button class=\\\"rgb-btn blue\\\" id=\\\"fan1-blue\\\" onclick=\\\"toggleFan1Blue()\\\">B</button>"
            "</div></div></div>"
            "<div class=\\\"fan\\\">"
            "<h3>Fan 2 (GPIO 21)</h3>"
            ...
            "</div>"
            "<div class=\\\"rgb-section\\\">"
            "<h4>RGB LEDs (GPIO 17)</h4>"
            "<div class=\\\"rgb-buttons\\\">"
            "<button class=\\\"rgb-btn red\\\" id=\\\"fan2-red\\\" onclick=\\\"toggleFan2Red()\\\">R</button>"
            "<button class=\\\"rgb-btn green\\\" id=\\\"fan2-green\\\" onclick=\\\"toggleFan2Green()\\\">G</button>"
            "<button class=\\\"rgb-btn blue\\\" id=\\\"fan2-blue\\\" onclick=\\\"toggleFan2Blue()\\\">B</button>"
            "</div></div></div></div>"
            "<script>"
            "var fan1_rgb={r:false,g:false,b:false};"
            "var fan2_rgb={r:false,g:false,b:false};"
            "function log(msg){document.getElementById('debug').textContent=new Date().toLocaleTimeString()+': '+msg;}"
            "function setFan1(speed){log('Setting Fan 1 to '+speed+'%');fetch('/set_fan1',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({speed:speed})}).then(r=>r.json()).then(d=>{if(d.success){log('Fan 1 set to '+speed+'%');updateFan1Buttons(speed);updateStatus();}}).catch(e=>log('Fan 1 Error: '+e));}"
            "function setFan2(speed){log('Setting Fan 2 to '+speed+'%');fetch('/set_fan2',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({speed:speed})}).then(r=>r.json()).then(d=>{if(d.success){log('Fan 2 set to '+speed+'%');updateFan2Buttons(speed);updateStatus();}}).catch(e=>log('Fan 2 Error: '+e));}"
            "function toggleFan1Red(){fan1_rgb.r=!fan1_rgb.r;updateFan1Color();}"
            "function toggleFan1Green(){fan1_rgb.g=!fan1_rgb.g;updateFan1Color();}"
            "function toggleFan1Blue(){fan1_rgb.b=!fan1_rgb.b;updateFan1Color();}"
            "function toggleFan2Red(){fan2_rgb.r=!fan2_rgb.r;updateFan2Color();}"
            "function toggleFan2Green(){fan2_rgb.g=!fan2_rgb.g;updateFan2Color();}"
            "function toggleFan2Blue(){fan2_rgb.b=!fan2_rgb.b;updateFan2Color();}"
            "function updateFan1Color(){var r=fan1_rgb.r?255:0;var g=fan1_rgb.g?255:0;var b=fan1_rgb.b?255:0;log('Fan 1 RGB: R='+fan1_rgb.r+' G='+fan1_rgb.g+' B='+fan1_rgb.b);fetch('/set_fan1_color',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({r:r,g:g,b:b})}).then(r=>r.json()).then(d=>{if(d.success){updateRgbButtons();}}).catch(e=>log('Fan 1 RGB Error: '+e));}"
            "function updateFan2Color(){var r=fan2_rgb.r?255:0;var g=fan2_rgb.g?255:0;var b=fan2_rgb.b?255:0;log('Fan 2 RGB: R='+fan2_rgb.r+' G='+fan2_rgb.g+' B='+fan2_rgb.b);fetch('/set_fan2_color',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({r:r,g:g,b:b})}).then(r=>r.json()).then(d=>{if(d.success){updateRgbButtons();}}).catch(e=>log('Fan 2 RGB Error: '+e));}"
            "function setArgbEffect(effect){log('Setting RGB effect to: '+effect);fetch('/set_argb_effect',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({effect:parseInt(effect)})}).then(r=>r.json()).then(d=>{if(d.success){log('RGB effect updated');}}).catch(e=>log('RGB Effect Error: '+e));}"
            "function updateRgbButtons(){document.getElementById('fan1-red').className='rgb-btn red'+(fan1_rgb.r?' on':'');document.getElementById('fan1-green').className='rgb-btn green'+(fan1_rgb.g?' on':'');document.getElementById('fan1-blue').className='rgb-btn blue'+(fan1_rgb.b?' on':'');document.getElementById('fan2-red').className='rgb-btn red'+(fan2_rgb.r?' on':'');document.getElementById('fan2-green').className='rgb-btn green'+(fan2_rgb.g?' on':'');document.getElementById('fan2-blue').className='rgb-btn blue'+(fan2_rgb.b?' on':'');}"
            "function updateFan1Buttons(activeSpeed){document.querySelectorAll('.fan1-btn').forEach(function(btn){btn.classList.remove('active');if(parseInt(btn.textContent)===activeSpeed){btn.classList.add('active');}});}"
            "function updateFan2Buttons(activeSpeed){document.querySelectorAll('.fan2-btn').forEach(function(btn){btn.classList.remove('active');if(parseInt(btn.textContent)===activeSpeed){btn.classList.add('active');}});}"
            "function updateStatus(){fetch('/status').then(r=>r.json()).then(d=>{document.getElementById('fan1-speed').textContent=d.fan1_speed+'%';document.getElementById('fan1-rpm').textContent=d.fan1_rpm;document.getElementById('fan2-speed').textContent=d.fan2_speed+'%';document.getElementById('fan2-rpm').textContent=d.fan2_rpm;document.getElementById('wifi').textContent=d.wifi_network;document.getElementById('ip').textContent=d.ip_address;document.getElementById('uptime').textContent=Math.floor(d.uptime/60)+'m';if(d.fan1_red!==undefined){fan1_rgb.r=d.fan1_red>0;fan1_rgb.g=d.fan1_green>0;fan1_rgb.b=d.fan1_blue>0;}if(d.fan2_red!==undefined){fan2_rgb.r=d.fan2_red>0;fan2_rgb.g=d.fan2_green>0;fan2_rgb.b=d.fan2_blue>0;}updateRgbButtons();updateFan1Buttons(d.fan1_speed||0);updateFan2Buttons(d.fan2_speed||0);log('Status OK');}).catch(e=>log('Status Error: '+e));}"
            "log('Fan Controller Ready');updateStatus();setInterval(updateStatus,2000);"
            "</script></body></html>";
